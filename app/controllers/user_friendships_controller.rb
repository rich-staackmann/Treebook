class UserFriendshipsController < ApplicationController
	...
	def accept
		...
		if @user_friendship.accept!
			...
			flash[:error] = "The friendship did not connect."
		end
	...
	def block
		...
		if @user_friendship.block!
			...
			flash[:error] = "The friendship could not be blocked."
		end
		redirect_to user_friendships_path
	end
		...
		if params[:user_friendship] && params[:user_friendship].has_key?(:friend_id)
			...
			respond_to do |format|
				if @user_friendship.new_record?
					format.html do
						flash[:error] = "There was a problem creating the friendship"
	...
	def friendship_association
		...
		when 'blocked'
			@user_friendships = current_user.blocked_user_friendships
		when 'pending'
			@user_friendships = current_user.pending_user_friendships
		when 'accepted'
			@user_friendships = current_user.accepted_user_friendships
		when 'requested'
			@user_friendships = current_user.requested_user_friendships
		end
	end
